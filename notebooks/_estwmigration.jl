using Pkg; Pkg.activate("/home/arzwa/dev/InfGenetics/")
using Distributed, StatsBase, Plots, Serialization, Distributions, Optim
addprocs(6)
@everywhere using InfGenetics

# estimate parameter of a censored geometric distribution by ML
function logpdf_censored_geometric(p, c, x)
    x < c ? log(p) + (x-1)*log(1-p) : (c-1)*log(1-p)
end

function mle_censored_geometric(c, xs)
    fun = p -> -sum(logpdf_censored_geometric.(Ref(p), Ref(c), xs))
    optimize(fun, 0.0, 1.0).minimizer
end

# Example        
# ========
function dosim(M=InfDemeMixEst(m=1.0, θ=fill(2.0, 3), U=Umat(0.05,0.05)))
    while true
        xs = simest(M)
        cs = counts(xs[2][end][2].c, 2:4)
        @info cs; cs[3] > cs[2] && return xs
    end
end

ex = dosim()

z_ = map(x->x.z, last.(ex[2]))
P1 = plot()
for i=1:length(z_)
    scatter!(fill(i, length(z_[i])), z_[i], color=:black, ms=2, label="")
end
plot!(legend=false, xlabel="generation", ylabel="\$z\$")
hline!([2])
P2 = plot(mapreduce(x->log10.(counts(x.c, 2:4)), hcat, last.(ex[2]))',
    legend=:topleft, marker=true, ms=2, ylabel="\$\\log_{10}N_k\$",
    xlabel="generation", label=["diploid" "triploid" "tetraploid"])
plot(P1, P2, size=(260*2,210), margin=3Plots.mm)


# Basic model
# ===========
nrep = 50_000
ms   = 10 .^ range(log10(0.01), stop=log10(3), length=10)
zs   = [2.5, 2.0, 1.5, 1.0]
γ    = 0.25
U    = Umat(0.05, 0.05)
nmax = 10000
Nest = 100
    
# get baseline results without migration (but with unreduced gametes)
base = map(zs) do z
    M  = InfDemeMixEst(m=0.0, θ=fill(z, 3), U=U, γ=γ)
    nr = 1_000_000
    Nest = 150
    x2 = map(_->simest2(M, x0=InfPop(z=[0.0], c=[2]), Nest=Nest), 1:nr)
    x4 = map(_->simest2(M, x0=InfPop(z=[0.0], c=[4]), Nest=Nest), 1:nr)
    p2 = sum(first.(x2) .&& map(x->argmax(x[3]) == 1, x2)) / nr
    p4 = sum(first.(x4) .&& map(x->argmax(x[3]) == 3, x4)) / nr
    (z=z, p2=p2, p4=p4)
end

sims = pmap(Iterators.product(ms, zs)) do (m, z)
    @info (m, z)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(z, 3), U=U), 
            nmax=nmax, Nest=Nest)
    end
    (m, z, reps)
end

#serialize("data/sims1-13-03-2025.jls", sims)
#sims = deserialize("data/sims1.jls")
#sims = deserialize("data/sims1b.jls")
sims = deserialize("data/sims1-21-12-2024.jls")
sims = deserialize("data/sims1-13-03-2025.jls")
base = deserialize("data/baseline2.jls")

function make_a_df(sims, nmax)
    map(eachcol(sims)) do col
        map(col) do xs 
            m, z, reps = xs
            ts = getindex.(reps, 2)
            tt = 1/mle_censored_geometric(nmax, ts)
            nsucc = sum(first.(reps))
            xx = sum(map(x->x[3][3] > x[3][1] && x[1], reps))
            p = xx/nsucc
            e1,e2 = jeffreys_interval(xx, nsucc, 0.05)
            (m=m, θ=z, nsucc=nsucc, n=xx, t=tt, p=p, p1=p-e1, p2=e2-p)
        end 
    end |> x->vcat(x...) |> DataFrame
end

dfa = make_a_df(sims, nmax)
dfb = make_a_df(sims, nmax)

df = vcat(dfa, dfb)

P1 = plot(title="(A) Time to establishment")
for gdf in groupby(df, :θ)
    gdf = sort(gdf, :m)
    plot!(gdf[:,:m], gdf[:,:t], marker=true, ms=2, label="\$\\theta=$(gdf[1,:θ])\$")
end
P1 = plot(P1, marker=true, ms=2, yscale=:log10, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, legend=:topleft, 
    ylim=(100,500_000), yticks=[100,1000,10000,100000])

P2 = plot(title="(B) Tetraploid establishment", xscale=:log10)
for gdf in groupby(df, :θ)
    gdf = sort(gdf, :m)
    plot!(gdf[:,:m], gdf[:,:p], ribbon=(gdf[:,:p1], gdf[:,:p2]), 
        marker=true, ms=2, label="\$\\theta=$(gdf[1,:θ])\$", fillalpha=0.2)
end
P2 = plot(P2, marker=true, ms=2, xlabel="\$m\$",
    ylabel="\$P_4\$", xscale=:log10, legend=:topright)
ppi = InfGenetics.cytotype_equilibrium(U)
hline!(ppi[[3]], label="",
    color=:black, xlabel="\$m\$", ylabel="\$P_4\$", ls=:dash)
for i=1:3
    hline!([(ppi[3]*base[i].p4)/(ppi[1]*base[i].p2 + ppi[3]*base[i].p4)], color=i, 
        label="")
end
plot!(bg_legend=:white, ylim=(0,0.09))

plot(P1, P2, size=(260*2,210), margin=3Plots.mm, titlefont=8)
#savefig("doc/img/estwmigration1c.pdf") |> texfig



# With double reduction etc. (maxα)
# =================================
nrep = 50_000
ms   = 10 .^ range(log10(0.01), stop=log10(3), length=10)
zs   = [2.0, 1.5, 1.0]
γ    = 0.25
U    = Umat(0.05, 0.05)
nmax = 10000
Nest = 100
α    = maxα
    
sims = pmap(Iterators.product(ms, zs)) do (m, z)
    @info (m, z)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(z, 3), U=U, α=maxα), 
            nmax=nmax, Nest=Nest)
    end
    (m, z, reps)
end

dfd = make_a_df(sims, nmax)
dfd = vcat(dfd, make_a_df(sims_, nmax))

#serialize("data/sims1-14-01-2025.jls", sims)
#sims = deserialize("data/sims1.jls")
#sims = deserialize("data/sims1b.jls")
#sims = deserialize("data/sims1-21-12-2024.jls")
base = deserialize("data/baseline2.jls")
sims_ = deserialize("data/sims1-21-12-2024.jls")

sims_  = deserialize("data/sims1-14-01-2025.jls")

P1 = plot(title="(A) Time to establishment")
for (i,_) in enumerate(eachcol(sims))
    for sim=[sims_, ]#sims]
        col = sim[:,i]
        t = map(col) do reps
            ts = getindex.(reps[3], 2)
            1/mle_censored_geometric(nmax, ts)
        end
        z = col[1][2]
        ms = first.(col)
        plot!(ms, t, marker=true, ms=2, label="\$\\theta=$z\$")
    end
end
P1 = plot(P1, marker=true, ms=2, yscale=:log10, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, legend=:topleft, 
    ylim=(100,500_000), yticks=[100,1000,10000,100000])

P2 = plot(title="(B) Tetraploid establishment", 
    xscale=:log10, xlabel="\$m\$", ylabel="\$P_4\$")
labs = ["\$\\alpha = [0,0,0]\$", "\$\\alpha = [1/2,1/4,1/6]\$"]
for (i,_) in enumerate(eachcol(sims))
    for (j,sim) in enumerate([sims_, sims])
        col = sim[:,i]
        xx = map(col) do reps
            nsucc = sum(first.(reps[3]))
            sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
        end
        x = first.(xx)
        n = last.(xx)
        @info n
        e = jeffreys_interval.(x, n, 0.05)
        p = x ./ n
        z = col[1][2]
        ms = first.(col)
        plot!(ms, p, color=i, ls=j == 1 ? :solid : :dot,
            ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
            marker=true, ms=2, label=j==1 ? "\$\\theta=$z\$" : "")
    end
end
ppi = InfGenetics.cytotype_equilibrium(U)
hline!(ppi[[3]], label="",
    color=:black, xlabel="\$m\$", ylabel="\$P_4\$", ls=:dash)
for i=1:3
    hline!([(ppi[3]*base[i].p4)/(ppi[1]*base[i].p2 + ppi[3]*base[i].p4)], color=i, 
        label="")
end
plot!(P2, deserialize("data/dronly.jls")..., color=2)
plot(P1, P2, size=(260*2,210), margin=3Plots.mm, titlefont=8)

savefig("doc/img/estwmigration-dr.pdf") |> texfig

# With only double reduction etc. (α = [0,0,1/6])
# ===============================================
nrep = 50_000
ms   = 10 .^ range(log10(0.1), stop=log10(3), length=10)
zs   = [1.5,]
γ    = 0.25
U    = Umat(0.05, 0.05)
nmax = 10000
Nest = 100
α    = [0,0,1/6]
    
sims = pmap(Iterators.product(ms, zs)) do (m, z)
    @info (m, z)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(z, 3), U=U, α=α), 
            nmax=nmax, Nest=Nest)
    end
    (m, z, reps)
        
ssims = [sims[:,2], sims_[:,2], sims__]

xs = map(ssims) do sims
    xx = map(sims) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    x = first.(xx)
    n = last.(xx)
    p = x ./ n
    ms = first.(sims)
    ms, p
end


data = collect(zip(xs[[2,1,3]], 
    [[0,0,0], [1/2,1/4,1/6],[0,0,1/6]]))

serialize("data/compare-dr.jls", data)

data = deserialize("data/compare-dr.jls")
plot()
map(data[[1,3,2]]) do (x, a)
    lab = join(["\$\\alpha_$k = " * @sprintf("%.2f\$", a[k-1]) for k=2:4], ", ")
    plot!(x..., label=lab, xscale=:log10, ms=2, marker=true)
end
plot!(xlabel="\$m\$", ylabel="\$P_4\$", size=(450,200), legend=:outertopright)

# With selfing
# =============
# This is not just with selfing, but with loss of SI...
nrep = 25_000
ms   = 10 .^ range(log10(0.01), stop=log10(3), length=7)
zs   = 1.5
σs   = [[NaN, s, s] for s=0:0.2:1.0]
γ    = 0.25
u    = 0.05
U    = Umat(u, u)
nmax = 10000
Nest = 100

sims = pmap(Iterators.product(ms, σs)) do (m, σ)
    @info (m, σ)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(zs, 3), U=U, σ=σ), 
            nmax=nmax, Nest=Nest)
    end
    (m, σ[3], reps)
end

#serialize("data/sims-si2-2024-08-20.jls", sims)
sims = deserialize("data/sims-si2-2024-08-20.jls")

P1 = plot(title="(A) Time to establishment")
for col in eachcol(sims)
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    s = col[1][2]
    plot!(ms, t, marker=true, ms=2, label="\$\\sigma = $s\$")
end
P1 = plot(P1, marker=true, ms=2, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, legend=false, yscale=:log10)

P2 = plot(title="(B) Tetraploid establishment", xscale=:log10,
    legend=:outertopright, xlabel="\$m\$", ylabel="\$P_4\$")
map(eachcol(sims)) do col
    xx = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    x = first.(xx)
    n = last.(xx)
    @info n
    e = jeffreys_interval.(x, n, 0.05)
    p = x ./ n
    s = col[1][2]
    plot!(ms, p, 
        ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
        marker=true, ms=2, label="\$\\sigma = $s\$")
end
#hline!([InfGenetics.cytotype_equilibrium(U)[3]], label="\$\\pi_4\$",
#    color=:black, xlabel="\$m\$", ylabel="\$P_4\$", ls=:dash, 
#    legend=:outertopright, size=(350,220))

plot(P1, P2, size=(540,200), layout=grid(1,2,widths=[0.415,0.585]),
    margin=3Plots.mm, titlefont=8)

savefig("doc/img/estwmigration-si2.pdf") |> texfig

P1 = plot()
for (j,col) in enumerate(eachcol(sims))
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    x = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    p = first.(x) ./ last.(x)
    s = col[1][2]
    scatter!(t[1:1], p[1:1], color=j, label="", ms=2)
    for i=1:length(col)-1
        plot!(t[i:i+1], p[i:i+1], color=j, marker=false, ms=2, 
            label=i==1 ? "\$\\sigma = $s\$" : "", arrow=(:closed, 2.0))
    end
end
P1 = plot(P1, marker=true, ms=2, ylabel="\$P_4\$", size=(400,300), grid=true,
    xlabel="\$\\overline{T}\$", cscale=:log10, legend=:outertopright, 
    title="\$\\bar{z}_s = $zs, \\gamma=$γ, u=v=$u\$")
#savefig("doc/img/estwmigration-si2.pdf") |> texfig


# With increased selfing (but no SI in diploids)
# ================================================
# random selfing assumed in diploids
nrep = 25_000
ms   = 10 .^ range(log10(0.01), stop=log10(3), length=7)
zs   = 1.5
stet = 0:0.2:1.0
σs   = [[0.0, st, st] for st=stet]
γ    = 0.25
u    = 0.05
U    = Umat(u, u)
nmax = 10000
Nest = 100

simsb = pmap(Iterators.product(ms, σs)) do (m, σ)
    @info (m, σ)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(zs, 3), U=U, σ=σ), 
            nmax=nmax, Nest=Nest)
    end
    (m, σ, reps)
end

#serialize("data/sims-selfing-notsi-2024-12-20.jls", sims)
serialize("data/sims-selfing-notsi-2025-01-03.jls", sims)
sims = deserialize("data/sims-selfing-notsi-2025-01-03.jls")

P1 = plot(title="(A) Time to establishment")
for col in eachcol(sims[:,:,1])
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    s = col[1][2]
    plot!(ms, t, marker=true, ms=2, label="\$\\sigma = $s\$")
end
P1 = plot(P1, marker=true, ms=2, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, legend=false, yscale=:log10)

map(1:5) do i
    plot(xcale=:log10, legend= i== 5 ? :topright : false, xlabel="\$m\$",
         ylabel="\$P_4\$", 
         title="\$\\sigma_2 = \\sigma_3 = $(@sprintf "%.2f" sdip[i])\$")
    map(eachcol(sims[:,i,:])) do col
        xx = map(col) do reps
            nsucc = sum(first.(reps[3]))
            sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
        end
        x = first.(xx)
        n = last.(xx)
        @info n
        e = jeffreys_interval.(x, n, 0.05)
        p = x ./ n
        s = col[1][2]
        plot!(ms, p, 
            ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
            marker=true, ms=2, label="\$\\sigma = $(@sprintf "%.2f" s[3])\$")
    end
    plot!()
end |> x->plot(x..., size=(600,400), xscale=:log10, ylim=(0,1))

savefig("doc/img/estwmigration-notsi.pdf") |> texfig

P1 = plot()
for (j,col) in enumerate(eachcol(sims))
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    x = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    p = first.(x) ./ last.(x)
    s = col[1][2]
    scatter!(t[1:1], p[1:1], color=j, label="", ms=2)
    for i=1:length(col)-1
        plot!(t[i:i+1], p[i:i+1], color=j, marker=false, ms=2, 
            label=i==1 ? "\$\\sigma = $s\$" : "", arrow=(:closed, 2.0))
    end
end
P1 = plot(P1, marker=true, ms=2, ylabel="\$P_4\$", size=(400,300), grid=true,
    xlabel="\$\\overline{T}\$", cscale=:log10, legend=:outertopright, 
    title="\$\\bar{z}_s = $zs, \\gamma=$γ, u=v=$u\$")
#savefig("doc/img/estwmigration-si2.pdf") |> texfig



# With assortative mating
# =======================
nrep = 25_000
ms   = 10 .^ range(log10(0.01), stop=log10(3), length=7)
zs   = 1.5
ρs   = [fill(r, 3) for r=0:0.2:1.0]
γ    = 0.25
U    = Umat(0.05, 0.05)
nmax = 10000
Nest = 100

simsc = pmap(Iterators.product(ms, ρs)) do (m, ρ)
    @info (m, ρ)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(zs, 3), U=U, ρ=ρ), 
            nmax=nmax, Nest=Nest)
    end
    (m, ρ[3], reps)
end

#serialize("data/sims-am2-2024-08-20.jls", sims)
sims = deserialize("data/sims-am2-2024-08-20.jls")

P1 = plot(title="(A) Time to establishment")
for col in eachcol(sims)
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    s = col[1][2]
    plot!(ms, t, marker=true, ms=2, label="\$\\rho = $s\$")
end
P1 = plot(P1, marker=true, ms=2, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, legend=false, yscale=:log10)

Nest = 100

simsc = pmap(Iterators.product(ms, ρs)) do (m, ρ)
    @info (m, ρ)
    reps = map(1:nrep) do _
        sim = simest2(InfDemeMixEst(m=m, θ=fill(zs, 3), U=U, ρ=ρ), 
            nmax=nmax, Nest=Nest)
    end
    (m, ρ[3], reps)
end

dfc = make_a_df(simsc, nmax, zs)

#serialize("data/sims-am2-2024-08-20.jls", sims)
simsc_ = deserialize("data/sims-am2-2024-08-20.jls")

P1 = plot(title="(A) Time to establishment")
for col in eachcol(sims)
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    s = col[1][2]
    plot!(ms, t, marker=true, ms=2, label="\$\\rho = $s\$")
end
P1 = plot(P1, marker=true, ms=2, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, legend=false, yscale=:log10)

P2 = plot(title="(B) Tetraploid establishment", xscale=:log10,
    xlabel="\$m\$", ylabel="\$P_4\$",
    legend=:outertopright, size=(350,220))
map(eachcol(sims)) do col
    xx = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    x = first.(xx)
    n = last.(xx)
    @info n
    e = jeffreys_interval.(x, n, 0.05)
    p = x ./ n
    s = col[1][2]
    plot!(ms, p, 
        ylim=(0,1),
        ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
        marker=true, ms=2, label="\$\\rho = $s\$")
end
#hline!([InfGenetics.cytotype_equilibrium(U)[3]], label="\$\\pi_4\$",
#    color=:black, xlabel="\$m\$", ylabel="\$P_4\$", ls=:dash, 
#    legend=:outertopright, size=(350,220))

plot(P1, P2, size=(540,200), layout=grid(1,2,widths=[0.415,0.585]),
    margin=3Plots.mm, titlefont=8)

savefig("doc/img/estwmigration-am2.pdf") |> texfig

P1 = plot()
for col in eachcol(sims)
    t = map(col) do reps
        1/mle_censored_geometric(nmax, getindex.(reps[3], 2))
    end
    x = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    p = first.(x) ./ last.(x)
    s = col[1][2]
    plot!(ms, t, p, marker=true, ms=2, label="\$\\rho = $s\$")
end
P1 = plot(P1, marker=true, ms=2, zlabel="\$P_4\$", size=(600,600), grid=true,
    ylabel="\$\\overline{T}\$", yscale=:log10, legend=false)


# combined plot SI/AM ---------------------------------------------------
sims = deserialize("data/sims-si2-2024-08-20.jls")
P3a = plot(title="(A)", legend=:outertopright)
map(eachcol(sims)) do col
    xx = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    x = first.(xx)
    n = last.(xx)
    @info n
    e = jeffreys_interval.(x, n, 0.05)
    p = x ./ n
    s = col[1][2]
    ms = first.(col)
    plot!(ms, p, 
        ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
        marker=true, ms=2, label="\$\\sigma_4 = $s\$")
end
sims = deserialize("data/sims-selfing-notsi-2025-01-03.jls")
map(enumerate(eachcol(sims))) do (i, col)
    xx = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    x = first.(xx)
    n = last.(xx)
    @info n
    e = jeffreys_interval.(x, n, 0.05)
    p = x ./ n
    s = col[1][2]
    ms = first.(col)
    plot!(ms, p, linestyle=:dot, color=i, alpha=0.3,
        ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
        marker=true, ms=2, label="")
end
plot!(xscale=:log10)
sims = deserialize("data/sims-am2-2024-08-20.jls")
P3b = plot(title="(B)", legend=:outertopright)
map(eachcol(sims)) do col
    xx = map(col) do reps
        nsucc = sum(first.(reps[3]))
        sum(map(x->x[3][3] > x[3][1] && x[1], reps[3])), nsucc
    end
    x = first.(xx)
βs   = [ones(3), [1, √(2/3), √(1/2)], [1, 3/4, 1/2]]
us   = [0.025, 0.05]

params = Iterators.product(ms, ρs, αs, βs, us)
@info length(params)

sims = pmap(params) do (m, ρ, α, β, u)
    @info (m, ρ, α[end], β[end], u)
    reps = map(1:nrep) do _
        M   = InfDemeMixEst(m=m, θ=fill(zs, 3), U=Umat(u,u), α=α, β=β, ρ=ρ)
        sim = simest2(M, nmax=nmax, Nest=Nest)
    end
    (M, reps)
end

sims = deserialize("data/sims-am2.jls")

P1 = plot(title="(A) Time to establishment")
for (r, col) in zip(ρs, eachcol(sims[:,:,1,1,1]))
    t = map(col) do (M, reps)
        1/mle_censored_geometric(nmax, getindex.(reps, 2))
    end
    plot!(ms, t, marker=true, ms=2, label="\$\\rho = $r\$")
end
P1 = plot(P1, marker=true, ms=2, xlabel="\$m\$",
    ylabel="\$\\overline{T}\$", xscale=:log10, 
    legend=false, yscale=:log10)

map(enumerate(αs)) do (i, α)
    P1 = plot(title="\$\\alpha = $α\$", xscale=:log10, yscale=:log10)
    P2 = plot(xscale=:log10)
    P3 = plot(xscale=:log10)
    for (j, (r, col)) in enumerate(zip(ρs, eachcol(sims[:,:,i,1,2])))
        t = map(col) do (M, reps)
            1/mle_censored_geometric(nmax, getindex.(reps, 2))
        end
        plot!(P1, ms, t, marker=true, ms=2, label="\$\\rho = $r\$", legend=false)
        xx = map(col) do (M, reps)
            nsucc = sum(first.(reps))
            sum(map(x->x[3][3] > x[3][1] && x[1], reps)), nsucc
        end
        x = first.(xx)
        n = last.(xx)
        e = jeffreys_interval.(x, n, 0.05)
        p = x ./ n
        @info n, p
        plot!(P2, ms, p, 
            label="\$\\rho=$(@sprintf "%.2f" r[1])\$",
            ribbon=(p .- first.(e), last.(e) .- p), fillalpha=0.2,
            marker=true, ms=2)
        plot!(P3, t, p, marker=true, ms=2, legend=false)
    end
    hline!(P2, [0.000731], color=:black, ls=:dash, label="",
        legend=:topleft, xlabel="\$m\$", ylabel="\$P_4\$")
    plot(P1, P2, P3, layout=(1,3))
end |> x->plot(x..., layout=(length(x),1), size=(750,length(x)*200))

plot(P1, P2, size=(550,200), margin=3Plots.mm)




