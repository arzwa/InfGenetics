...
%DIF DEL submission2.tex   Tue May 13 16:34:34 2025
%DIF ADD paper.tex         Tue May 13 16:34:20 2025
    \input{preamble.tex}
...
\begin{document}
    ...
    gene copy in individual $i$ is derived from parent $k$.
    ...
    In generation $t$, \DIFdelbegin \DIFdel{$M(t)$ }\DIFdelend \DIFaddbegin \DIFadd{$M(t) \sim \mathrm{Poisson}(m)$ }\DIFaddend migrant individuals
    arrive on an island (the new habitat) joining $N^\ast(t)$ resident
    individuals\DIFdelbegin \DIFdel{, where $M(t)$ is Poisson
    distributed with mean $m$, and }\DIFdelend \DIFaddbegin \DIFadd{.
    We assume }\DIFaddend $N^\ast(0)=0$ unless stated otherwise. 
    ...
    island\DIFdelbegin \DIFdel{which reproduce sexually , and the offspring thus produced survives until the
    next generation }\DIFdelend \DIFaddbegin \DIFadd{.
    ...
    zygotes, of which $N^\ast(t+1)$ survive to reproductive age }\DIFaddend with a
    probability \DIFdelbegin \DIFdel{determined by their trait value.
    Note that this corresponds to a life cycle where selection occurs before
    migration within a generation.
    In the basic model, random selfing is
    allowed (but see below for a model with self-incompatibility)}\DIFdelend \DIFaddbegin \DIFadd{proportional to individual fitness, where $N^\ast(t+1)$ is
    Poisson distributed with mean $N(t)\overline{W}$ and $\overline{W}$ is the
    mean fitness among zygotes}\DIFaddend .
    We assume the trait \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{to be }\DIFaddend under directional selection, with \DIFdelbegin \DIFdel{fitness }\DIFdelend \DIFaddbegin \DIFadd{the fitness of a
    zygote with trait value $z$ given by }\DIFaddend $w(z) = e^{\gamma(z - \theta)}$, where
    ...
    \DIFaddbegin \DIFadd{Note that this model corresponds to a life cycle where selection occurs
    before migration within a generation.
}\DIFaddend 

    Again following \cite{barton2018}, we simulate the model by first calculating
    the fitness of each parental pair $(i,j)$, which is \DIFaddbegin \DIFadd{defined as }\DIFaddend the expected
    fitness of \DIFdelbegin \DIFdel{offspring of this pair
    }\DIFdelend \DIFaddbegin \DIFadd{a random zygote derived from this pair
    }\DIFaddend \begin{equation}
      w_{ij}
        = \sum_{k=1}^2\sum_{l=1}^2 w_{ij}^{kl}
        = \sum_{k=1}^2\sum_{l=1}^2 u_{c_{i},k}u_{c_{j},l}
            \Ex\left[e^{\gamma(Z_{ij}^{kl} - \theta)}\right] \DIFaddbegin \label{eq:wij}
    \DIFaddend \end{equation}
    The \DIFaddbegin \DIFadd{expected number of offspring individuals of an individual $i$
    surviving to reproductive age is then $w_i = \frac{1}{N(t)}\sum_j w_{ij}$.
    The }\DIFaddend expectation on the right hand side \DIFaddbegin \DIFadd{of }\cref{eq:wij} \DIFaddend can be calculated
    ...
    generation \DIFdelbegin \DIFdel{is calculated as $N^\ast(t+1) = \sum_{i,j}w_{ij}/N(t)$.
    Next, $N^\ast(t+1)$ offspring individuals are obtained }\DIFdelend \DIFaddbegin \DIFadd{$N^\ast(t+1)$ is sampled from a Poisson distribution with mean
    $N(t)\overline{W} = \sum_i w_i = \sum_{i,j}w_{ij}/N(t)$.
    Offspring cytotypes and trait values are then determined }\DIFaddend by sampling
    parental pairs and gametes proportional to $w_{ij}^{kl}$, and sampling a
    trait value from \DIFdelbegin \DIFdel{a Gaussian distribution }\DIFdelend \DIFaddbegin \DIFadd{the offspring trait value distribution conditional on
    survival to reproductive age, which is Gaussian }\DIFaddend with mean
...
For instance, it could be of interest to develop \DIFdelbegin \DIFdel{a complicated }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend individual-based
model along the lines of \cite{oswald2011} to study \DIFdelbegin \DIFdel{the effects of }\DIFdelend \DIFaddbegin \DIFadd{competitive exclusion
versus coexistence of cytotypes in mixed-ploidy populations with partial
}\DIFaddend selfing, assortative mating\DIFdelbegin \DIFdel{and competitionon establishment and coexistence of
tetraploids within diploid populations}\DIFdelend \DIFaddbegin \DIFadd{, competition, }\textit{\DIFadd{etc.}}\DIFaddend , but where the focal
trait that determines \DIFdelbegin \DIFdel{fitness and assortative mating }\DIFdelend \DIFaddbegin \DIFadd{the various fitness components }\DIFaddend is not controlled by a few
