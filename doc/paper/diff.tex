...
%DIF ADD paper.tex         Thu Mar 27 22:00:30 2025
\input{preamble.tex}
...
\citep{husband1997,ronfort1999,otto2000,husband2008,clo2022b}}\hspace{0pt}%DIFAUXCMD
...
an appreciable size as it evades to some extent the negative effects of MCE}\DIFaddend .
...
$u$ is
the probability that a diploid produces an unreduced gamete in meiosis),
so that tetraploids need a substantial advantage during colonization if they
are to establish before diploids do}\DIFdelend \DIFaddbegin \DIFadd{More often than not, local adaptation is polygenic in nature
\mbox{%DIFAUXCMD
\citep{pritchard2010,barghi2020,bomblies2022}}\hspace{0pt}%DIFAUXCMD
, involving many weakly selected
variants across the genome, and adaptation during polyploid establishment is
...
tetraploid populations relates to adaptation that occurred during initial
...
%DIFDELCMD < %%%
\DIFdelend Here we develop a model for the establishment of a mixed-ploidy population in a
...
Indeed, in the case where allelic effects are scaled across ploidy levels to
yield the same equilibrium genetic variance ($\beta_4 = \sqrt{1/2}$\DIFdelbegin \DIFdel{, so that
polyploid migrants are not more likely to have high fitness than diploid
migrants}\DIFdelend ), the
establishment probability for tetraploids can be almost five times as high as
\DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend diploids depending on the selection gradient (\cref{fig:est1}A).
...
\DIFdelbegin \DIFdel{While }\DIFdelend \DIFaddbegin \DIFadd{As expected, }\DIFaddend the probability of establishment \DIFdelbegin \DIFdel{becomes lower as }\DIFdelend \DIFaddbegin \DIFadd{decreases when }\DIFaddend the strength of
selection becomes large, \DIFdelbegin \DIFdel{the establishment probability does not decrease
monotonically with }\DIFdelend \DIFaddbegin \DIFadd{as the fitness of the initial migrant individual
decreases with increasing }\DIFaddend $\gamma$ \DIFdelbegin \DIFdel{, i.e.
as }\DIFdelend \DIFaddbegin \DIFadd{(}\cref{fig:est1}\DIFadd{A).
However, when }\DIFaddend selection becomes very weak, the establishment probability also
decreases.
...
probability to reach $N=100$ is \DIFdelbegin \DIFdel{$1/100$ }\DIFdelend \DIFaddbegin \DIFadd{$\sim 1/100$ }\DIFaddend \citep{barton2018}.  
...
\begin{figure}[t]
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.8\linewidth]{/home/arzwa/dev/InfGenetics/doc/img/estwmigration-dr.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\linewidth]{/home/arzwa/dev/InfGenetics/doc/paper/figs/estwmig.pdf}
\DIFaddendFL \caption{
...
\begin{figure}
\centering
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.9\linewidth]{/home/arzwa/dev/InfGenetics/doc/img/prez.pdf} 
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.9\linewidth]{/home/arzwa/dev/InfGenetics/doc/paper/figs/prez.pdf} 
    \DIFaddendFL \caption{
...
differ systematically across ploidy levels \mbox{%DIFAUXCMD
\citep{porturas2019}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
variance \DIFdelbegin \DIFdel{is constant }\DIFdelend across cytotypes ($\beta_4=\sqrt{1/2}$ in our model)\DIFdelbegin \DIFdel{.
...
variance across ploidy levels may often be fairly constant, so our assumption
may not yield too bad an approximation.
It should be noted that other authors have made different assumptions
on how allelic effects (and hence genetic variance) scale across ploidy levels
(in particular \mbox{%DIFAUXCMD
\cite{griswold2021}}\hspace{0pt}%DIFAUXCMD
, who scaled allelic effects in a way that
is equivalent to $\beta_4 = 1/2$ in our model).
...
large-effect loci, but many loci of small effect.
