...
We show that for rare migration from the source population, autotetraploids are
more likely to found a successful population than diploids under a very
...
Here we develop a rather general model for the establishment of a mixed-ploidy
population in a novel, unoccupied habitat based on \cite{barton2018}.
...
In a finite population, the segregation variance will decay over time
...
are as in diploids, but scaled by a factor $\beta_k$, and assume equal allele
frequencies in the different cytotypes, 
we find that
\begin{equation}
...
\textit{twice} the excess of their parent on average, unless allelic effects are
scaled exactly by one half (i.e. $\beta_4 = 1/2$). 
...
variance (as it increases the segregation variance), but leads to accelerated
inbreeding, causing faster loss of genetic variance in the long-term
(\cref{fig:vztet,s-fig:fininf}).
...
course also depend on how allelic effects scale across ploidy levels ($\beta$).
...
diploids are assumed to be self-compatible with $\sigma=0$ in diploids (i.e.
random selfing, dashed transparent lines).
The tetraploid establishment probability is still markedly increased when
$\sigma \ge 0.4$, and as for the simulations with self-incompatibility,
...
a model of inbreeding depression.
...
and migration load (where inbreeding depression is modeled as an immediate
fitness reduction in offspring produced by selfing).
...
It would be very interesting to combine the infinitesimal framework with a
simple model of inbreeding depression to investigate in a somewhat more
realistic model whether the combination of maladaptive migration and
differential inbreeding depression could explain the prevalence of polyploid
subpopulations at range edges.
...
diploid counterparts, in which case polyploidy would lead to reduced rather
than increased inbreeding depression (as discussed above).
