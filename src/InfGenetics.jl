...
function generate_offspring(d::InfDemeTetra) 
    ...
    P = zeros(size(d.F))
    N = length(d)
    # can we store D and have an update rule? so that G becomes superfluous?
    D = [(d.F[k,k] + d.F[l,l] + 4d.F[k,l])/6 for k=1:N, l=1:N]
    for k=1:N
        i = rand(1:N)
        j = rand(1:N)
        Cij = d.V * (d.G[i] + d.G[j]) / 2  # this could be in terms of the previous `D`?
        zmp = (d.z[i] + d.z[j]) / 2
        P[k,i] += 0.5
        P[k,j] += 0.5
    ...
    F = P * (d.F + (0.25I - Diagonal(d.F)/4)) * P'  # verify
    F[diagind(F)] .= diag(P * D * P')
    #return (F=F, D=D, P=P, G=G, z=z)
    return InfDemeTetra(z, G, F, d.V)
end

function evolve_and_track_segvar(d::InfDemeTetra, t)
    map(1:t) do gen
